...
def list_forms(is_active: bool = None, api_key: str = None) -> dict:
    ...
    try:
        resp = requests.get(f"{_BASE}/form/", headers=_headers(api_key), params=params)
        return resp.json()
...
def retrieve_form(form_id: str, api_key: str = None) -> dict:
    ...
    try:
        resp = requests.get(f"{_BASE}/form/{form_id}/", headers=_headers(api_key))
        return resp.json()
    except requests.RequestException as e:
        return {"error": f"API request failed: {str(e)}"}
...
def update_form(form_id: str, name: str = None, is_active: bool = None, api_key: str = None) -> dict:
    ...
    try:
        resp = requests.patch(f"{_BASE}/form/{form_id}/", headers=_headers(api_key), json=body)
        return resp.json()
    except requests.RequestException as e:
        return {"error": f"API request failed: {str(e)}"}
...
def delete_form(form_id: str, api_key: str = None) -> dict:
    ...
    if not form_id:
        return {"error": "form_id is required"}
    try:
        resp = requests.delete(f"{_BASE}/form/{form_id}/", headers=_headers(api_key))
